const ourStory = {
  intro:
    'W branży budowlanej działamy od 2000 roku. Doświadczenie zdobywaliśmy na licznych inwestycjach w kraju i za granicą.',
  suddenly: 'Aż pewnego razu...',
  slides: [
    {
      enlightenment: { line1: 'Doznałem ', line2: 'olśnienia!' },
      text1:
        'To był dzień jak codzień, ręce pełne roboty. Wszystko szło pięknie, choć przed południem dała o sobie znać jakaś niepomyślna passa.',
    },
    {
      text1:
        'Właśnie wchodziłem na najwyższa kondygnaję budynku, nucąć dla pocieszenia',
      title: ' Always look on the bright side of life ',
      text2: 'gdy nagle oślepiło mnie Słońce.',
    },
    {
      text1:
        'W pierwszym odruchu "wyszeptałem" "zaklęcie" doskonale znane każdemu człowiekowi czynu budowlanego. Zrobiłem krok.. I nastąpił przełom (zaklęcie zadziałało ;-).',
    },
    {
      text1:
        'Zatrzymałem się, bo oto dotarła do mnie wpaniała myśl... Zapragnąłem mieć Słońce w swojej ekipie, współpracować z Nim.',
    },
  ],

  //   slides: [
  //     {
  //       enlightenment: { line1: 'Doznałem ', line2: 'olśnienia!' },
  //       text1:
  //         'To był dzień jak codzień, ręce pełne roboty. Wszystko szło pięknie, choć przed południem dała o sobie zanć jakaś niepomyślna passa.',
  //     },
  //     {
  //       text1:
  //         'Właśnie wchodziłem na najwyższa kondygnaję budynku, nucąć dla pocieszenia',
  //       title: ' Always look on the bright side of life ',
  //       text2: 'gdy nagle oślepiło mnie Słońce.',
  //     },
  //     {
  //       text1:
  //         'W pierwszym odruchu "wyszeptałem" "zaklęcie" doskonale znane każdemu człowiekowi czynu budowlanego. Zrobiłem krok.. I nastąpił przełom (zaklęcie zadziałało ;-).',
  //     },
  //     {
  //       text1:
  //         'Zatrzymałem się, bo oto dotarła do mnie wpaniała myśl... Zapragnąłem mieć Słońce w swojej ekipie, współpracować z Nim.',
  //     },
  //   ],
  conclusion: 'Dlatego powstało duploELEMENTS!',
};

export { ourStory };
